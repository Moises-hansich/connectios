...
export function HardwareForm({
  ...
  const [formData, setFormData] = useState<HardwareFormData>(estadoInicial);
  ...
  const tipoSelecionado = useMemo(() => {
    return tiposHardware.find((tipo) => tipo.id === tipoSelecionadoId);
  ...
  async function carregarCamposDoTipo(tipoHardwareId: number) {
    try {
      ...
      const dados = await campoHardwareService.listarPorTipo(tipoHardwareId);
  ...
  function validarFormulario(): boolean {
    ...
    if (campoObrigatorioVazio) {
      toast.error(`O campo "${campoObrigatorioVazio.nome}" é obrigatório.`);
  ...
  return (
    <form onSubmit={salvarHardware} className="space-y-6">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <FormField label="Nome" obrigatorio className="md:col-span-2">
          <input
            type="text"
            name="nome"
            value={formData.nome}
        ...
        <FormField label="Tipo de hardware" obrigatorio>
          ...
          >
            <option value="">
              {carregandoTipos ? "Carregando tipos..." : "Selecione um tipo"}
            </option>
        ...
        <section className="rounded-xl border border-slate-200 bg-slate-50 p-5">
          ...
          {carregandoCampos ? (
            ...
            <div className="rounded-lg border border-dashed border-slate-300 bg-white p-6 text-center text-sm text-slate-500">
              Nenhum campo adicional foi cadastrado para este tipo de hardware.
            </div>
          ) : (
            <div className="grid gap-5 md:grid-cols-2">
              {camposHardware.map((campo) => (
                <CampoDinamico
                  ...
                  disabled={salvando}
                  onChange={(valor) => alterarValorDinamico(campo.id, valor)}
                />
              ))}
            </div>
          )}
      ...
      <div className="flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end">
        ...
        <Button type="submit" loading={salvando} disabled={carregandoCampos}>
          {!salvando && <Save size={18} />}

          {editando ? "Salvar alterações" : "Cadastrar hardware"}
        </Button>
      </div>
    </form>
  );
...
function CampoDinamico({
  ...
  if (tipo === "boolean" || tipo === "booleano" || tipo === "checkbox") {
    return (
