...
function FormatacaoCampo({ campo }: { campo: CampoHardware }) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span
        className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${obterCorTipoDado(
          campo.tipoDado,
        )}`}
      >
      ...
      {grupos.map((grupo) => (
        ...
        >
          <header className="flex items-center justify-between gap-4 border-b border-slate-200 bg-slate-50 px-5 py-4">
            ...
            <span className="shrink-0 rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-600 shadow-sm ring-1 ring-slate-200">
              {grupo.campos.length}{" "}
